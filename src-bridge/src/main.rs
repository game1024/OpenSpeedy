...
use windows::Win32::System::Memory::{
    VirtualAllocEx, VirtualFreeEx, MapViewOfFile, UnmapViewOfFile, OpenFileMappingW,
    FILE_MAP_ALL_ACCESS, FILE_MAP_READ, MEM_COMMIT, MEM_RELEASE, MEM_RESERVE, PAGE_READWRITE,
};
...
fn do_enable(pid: u32) -> Result<(), String> {
    let name = format!("OpenSpeedy.{pid}");
    let name_wide = to_wide(&name);
    let h_map = unsafe {
        OpenFileMappingW(FILE_MAP_ALL_ACCESS.0, false, PCWSTR::from_raw(name_wide.as_ptr()))
    }.map_err(|_| "OpenFileMappingW failed — DLL injected?")?;

    let p = unsafe { MapViewOfFile(h_map, FILE_MAP_ALL_ACCESS, 0, 0, 1) };
    if p.Value.is_null() { unsafe { let _ = CloseHandle(h_map); }; return Err("MapViewOfFile failed".into()); }
    unsafe { *(p.Value as *mut bool) = true; UnmapViewOfFile(p); }
    unsafe { let _ = CloseHandle(h_map); }
    Ok(())
}
...
fn do_disable(pid: u32) -> Result<(), String> {
    ...
    let p = unsafe { MapViewOfFile(h_map, FILE_MAP_ALL_ACCESS, 0, 0, 1) };
    if p.Value.is_null() { unsafe { let _ = CloseHandle(h_map); }; return Err("MapViewOfFile failed".into()); }
    unsafe { *(p.Value as *mut bool) = false; UnmapViewOfFile(p); }
    unsafe { let _ = CloseHandle(h_map); }
    Ok(())
}
...
fn do_is_enabled(pid: u32) -> Result<bool, String> {
    ...
    let p = unsafe { MapViewOfFile(h_map, FILE_MAP_READ, 0, 0, 1) };
    if p.Value.is_null() { unsafe { let _ = CloseHandle(h_map); }; return Err("MapViewOfFile failed".into()); }
    let enabled = unsafe { *(p.Value as *const bool) };
    unsafe { UnmapViewOfFile(p); let _ = CloseHandle(h_map); }
    Ok(enabled)
}
